...
class ReadonlyRestClient extends ServerActions {
    ...
    listen(query, currentHashFn, tag, onComplete) {
        ...
        const queryStringParameters = queryParamsToRestQueryStringParameters(query.getQueryParams());
...
function syncPointRemoveEventRegistration(syncPoint, query, eventRegistration, cancelError) {
    ...
    if (hadCompleteView && !syncPointHasCompleteView(syncPoint)) {
        ...
        removed.push(new (syncPointGetReferenceConstructor())(query.database, query.path));
    }
...
function syncTreeGetServerValue(syncTree, query) {
    ...
    const serverCacheNode = serverCacheComplete
        ...
        : null;
    const writesCache = writeTreeChildWrites(syncTree.pendingWriteTree_, query.path);
 ...
 */
class Repo {
    constructor(repoInfo_, forceRestClient_, app, authTokenProvider_) {
        this.repoInfo_ = repoInfo_;
...
function repoStart(repo) {
    ...
    if (repo.forceRestClient_ || beingCrawled()) {
        ...
        repo.persistentConnection_ = new PersistentConnection(repo.repoInfo_, repo.app.options.appId, (pathString, data, isMerge, tag) => {
            repoOnDataUpdate(repo, pathString, data, isMerge, tag);
        }, (connectStatus) => {
...
function repoGetValue(repo, query) {
    ...
    return repo.server_.get(query).then(payload => {
        ...
        const events = syncTreeApplyServerOverwrite(repo.serverSyncTree_, query.path, node);
 ...
 */
class Query {
    constructor(database, path, queryParams_, orderByCalled_) {
    ...
    getRef() {
        ...
        // method gets called.
        return new Query.__referenceConstructor(this.database, this.path);
    }
     ...
     */
    get() {
        return repoGetValue(this.repo, this).then(node => new DataSnapshot(node, this.getRef(), this.getQueryParams().getIndex()));
    }
    /**
    ...
    limitToFirst(limit) {
        ...
        return new Query(this.database, this.path, queryParamsLimitToFirst(this.queryParams_, limit), this.orderByCalled_);
    }
    /**
    ...
    limitToLast(limit) {
        ...
        return new Query(this.database, this.path, queryParamsLimitToLast(this.queryParams_, limit), this.orderByCalled_);
    }
    /**
    ...
    orderByChild(path) {
        ...
        return new Query(this.database, this.path, newParams, 
        /*orderByCalled=*/ true);
    }
    /**
    ...
    transaction(transactionUpdate, onComplete, applyLocally) {
        ...
        const promiseComplete = (error, committed, node) => {
            if (error) {
                ...
                const snapshot = new DataSnapshot(node, new Reference(this.database, this.path), PRIORITY_INDEX);
        ...
        repoStartTransaction(this.repo, this.path, transactionUpdate, promiseComplete, unwatcher, applyLocally);
        return deferred.promise;
    }
...
class FirebaseDatabase {
    constructor(app, authProvider, databaseUrl) {
        this.app = app;
        this._delegate = repoManagerDatabaseFromApp(this.app, authProvider, databaseUrl, undefined);
    }
    /**
     * Modify this instance to communicate with the Realtime Database emulator.
     *
...
export { ServerValue, enableLogging, getDatabase };
